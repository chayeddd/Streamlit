...
def create_globe_icon():
    fig, ax = plt.subplots(figsize=(0.1, 0.1), dpi=100)  # Adjust the figsize to make the globe smaller
    ax.set_aspect('equal')
    ax.set_axis_off()

    # Draw the globe (circle)
    circle = plt.Circle((0.5, 0.5), 0.4, color='deepskyblue', ec='black', lw=0.5)
    ax.add_patch(circle)
